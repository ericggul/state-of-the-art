...
const Path = React.memo(({ d, strokeWidth }) => {
  const [opacity, setOpacity] = useState(Math.random());

  useRandomInterval(() => setOpacity(Math.random()), 100, 3000);

  return <path d={d} stroke="white" fill="none" strokeWidth={strokeWidth} opacity={opacity} />;
});
