...
export default function TextGrid({ isTesting = false, activated = false }) {
  ...
  const [numbers, setNumbers] = useState(new Array(200).fill(0).map((_, i) => getRandomMax(iterationAfterActivatedRef.current * ACCELERATION + 0.5)));

  useEffect(() => {
    if (!localActivated) {
      iterationAfterActivatedRef.current = 0;
      setNumbers(new Array(200).fill(" "));
      return;
    }
