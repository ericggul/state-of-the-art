...
export default function Layer1({ newEmbeddings }) {
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force("charge", d3.forceManyBody().strength(-10 * ((width + height) / 1000) ** 2))
      .force("center", d3.forceCenter())
      // .force("boundary", forceBoundary(-width * 0.5, -height * 0.45, width * 0.5, height * 0.45))

      .force("boundary", forceBoundary(-width, -height, width * 0.1, height * 0.1))
    ...
    const link = svg
      ...
      .attr("stroke-width", width * 0.0024)
      .attr("opacity", "0.27");
