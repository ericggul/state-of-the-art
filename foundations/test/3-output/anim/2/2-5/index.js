...
const BASE_CYCLE_DURATION = 700; // Base duration of one full cycle in milliseconds
const RANDOM_FACTOR = 1.0; // Amount of randomness to add

// Define different cycle durations for each parameter
...
export default function Layer3({ newResponse }) {
  ...
  useEffect(() => {
    const updateBezierParams = () => {
      ...
      setBezierParams({
        controlX1Factor: getCyclicalValue(time, -0.5, 1.5, CYCLE_DURATIONS.controlX1Factor) + getRandom(-RANDOM_FACTOR, RANDOM_FACTOR),
        controlX2Factor: getCyclicalValue(time, -0.5, 1.5, CYCLE_DURATIONS.controlX2Factor) + getRandom(-RANDOM_FACTOR, RANDOM_FACTOR),
        controlY1Factor: getCyclicalValue(time, -4, 4, CYCLE_DURATIONS.controlY1Factor) + getRandom(-RANDOM_FACTOR, RANDOM_FACTOR),
...
const Tokens = React.memo(function Tokens({ logProbs, wordPosCalc }) {
  return (
    <S.Tokens>
      {logProbs.map((tokenData, i) => (
        <Token key={i} xIdx={i} token={tokenData.token} logprobs={tokenData.top_logprobs} wordPosCalc={wordPosCalc} />
      ))}
    </S.Tokens>
  );
});
...
const Token = React.memo(function Token({ xIdx, token, logprobs, wordPosCalc }) {
  const topLogProbs = logprobs.length > 0 ? logprobs : [{ token: token, percentage: 100 }];

  return (
    <>
      <S.Candidate style={{ left: wordPosCalc(xIdx, -1)[0], top: wordPosCalc(xIdx, -1)[1] }}>{token}</S.Candidate>
      {topLogProbs.map((target, yIdx) => (
        <S.Candidate
          key={yIdx}
          style={{
