import { Canvas } from "@react-three/fiber";
import { OrbitControls, Environment, Instances, Instance } from "@react-three/drei";
import { useMemo, Suspense } from "react";

// Number of encoder and decoder layers for DALL·E 2
const NUM_ENCODER_LAYERS = 12; // Simplified Text Encoder (CLIP-like)
const NUM_DIFFUSION_LAYERS = 16; // Simplified Diffusion Decoder (UNet-like)

// DALL·E 2 structure definition
const STRUCTURE = [];

// Generate Text Encoder layers (similar to a transformer encoder)
STRUCTURE.push({ name: `Input Text Embeddings`, type: "input", stack: "encoder" }, { name: `Positional Encoding (Text)`, type: "positional", stack: "encoder" });

// Encoder blocks (Transformer-based CLIP-like architecture)
for (let i = 1; i <= NUM_ENCODER_LAYERS; i++) {
  STRUCTURE.push({
    name: `Encoder Layer ${i}`,
    type: "encoder_layer",
    stack: "encoder",
    sublayers: [
      { name: `Self-Attention ${i}`, type: "attention", dimensions: [768, 12, 12] },
      { name: `Feed Forward ${i}`, type: "ffn", dimensions: [3072, 12, 1] },
    ],
  });
}

// Generate Diffusion Decoder layers (UNet-based architecture)
STRUCTURE.push({ name: `Noise Input (Image Latents)`, type: "input", stack: "decoder" });

// Diffusion layers (convolutional and upsampling blocks)
for (let i = 1; i <= NUM_DIFFUSION_LAYERS; i++) {
  STRUCTURE.push({
    name: `Diffusion Layer ${i}`,
    type: "diffusion_layer",
    stack: "decoder",
    sublayers: [
      { name: `Conv Layer ${i}`, type: "conv", dimensions: [128, 8, 8] }, // Convolutional layers
      { name: `Upsample Layer ${i}`, type: "upsample", dimensions: [64, 8, 8] }, // Upsampling layers
    ],
  });
}

// Final Output Image layer
STRUCTURE.push({ name: "Final Image Projection", type: "output", stack: "decoder" });

// Uniform color scheme with hue 240
const COLORS = {
  default: "hsl(240, 100%, 50%)",
};

export default function DALLE2Visualization() {
  const encoderLayers = STRUCTURE.filter((layer) => layer.stack === "encoder");
  const decoderLayers = STRUCTURE.filter((layer) => layer.stack === "decoder");

  const layerHeight = 10; // Vertical spacing between layers

  return (
    <Canvas
      camera={{
        position: [0, (NUM_ENCODER_LAYERS + NUM_DIFFUSION_LAYERS) * layerHeight * 0.5, (NUM_ENCODER_LAYERS + NUM_DIFFUSION_LAYERS) * layerHeight * 1.2],
        fov: 50,
        near: 0.1,
        far: 5000,
      }}
    >
      <Suspense fallback={null}>
        <Environment preset="apartment" />
      </Suspense>
      <pointLight position={[0, 200, 0]} intensity={1} />
      <directionalLight position={[0, 150, 100]} intensity={1} />
      <ambientLight intensity={0.5} />

      {/* Encoder (Text) Stack */}
      {encoderLayers.map((layer, i) => {
        const y = i * layerHeight - (encoderLayers.length * layerHeight) / 2 + layerHeight / 2;
        return <Layer key={`encoder-${i}`} position={[-50, y, 0]} layer={layer} color={COLORS.default} index={i} />;
      })}

      {/* Decoder (Diffusion) Stack */}
      {decoderLayers.map((layer, i) => {
        const y = i * layerHeight - (decoderLayers.length * layerHeight) / 2 + layerHeight / 2;
        return <Layer key={`decoder-${i}`} position={[50, y, 0]} layer={layer} color={COLORS.default} index={i} />;
      })}

      <OrbitControls enablePan={true} maxPolarAngle={Math.PI / 2} />
    </Canvas>
  );
}

const Layer = ({ position, layer, color }) => {
  const size = [30, 10, 10]; // Base size for layers
  const gap = 10; // Gap between sublayers

  // For layers that have sublayers (encoder_layer, diffusion_layer)
  if (layer.sublayers) {
    return (
      <group position={position}>
        {layer.sublayers.map((sublayer, idx) => (
          <Sublayer key={`${layer.name}-sublayer-${idx}`} position={[0, (idx - (layer.sublayers.length - 1) / 2) * (size[1] + gap), 0]} sublayer={sublayer} color={color} />
        ))}
      </group>
    );
  }

  // For layers without sublayers
  return (
    <group position={position}>
      <Node size={size} color={color} />
    </group>
  );
};

const Sublayer = ({ position, sublayer, color }) => {
  const size = [20, 8, 8]; // Size for sublayers
  const gridConfig = {
    attention: { xCount: 12, yCount: 12, xInterval: 3, yInterval: 3 },
    ffn: { xCount: 24, yCount: 4, xInterval: 2, yInterval: 5 },
    conv: { xCount: 8, yCount: 8, xInterval: 4, yInterval: 4 },
    upsample: { xCount: 4, yCount: 4, xInterval: 5, yInterval: 5 },
  };

  const grid = gridConfig[sublayer.type] || { xCount: 1, yCount: 1, xInterval: 0, yInterval: 0 };

  return (
    <group position={position}>
      <InstancedNodes
        xCount={grid.xCount}
        yCount={grid.yCount}
        xInterval={grid.xInterval}
        yInterval={grid.yInterval}
        nodeSize={[size[0] / grid.xCount, size[1] / grid.yCount, size[2]]}
        color={color}
      />
    </group>
  );
};

const Node = ({ size, color }) => {
  return (
    <mesh>
      <boxGeometry args={size} />
      <meshStandardMaterial color={color} roughness={0.3} metalness={0.5} opacity={0.6} transparent={true} />
    </mesh>
  );
};

const InstancedNodes = ({ xCount, yCount, xInterval, yInterval, nodeSize, color }) => {
  const positions = useMemo(() => {
    const temp = [];
    for (let i = 0; i < xCount; i++) {
      for (let j = 0; j < yCount; j++) {
        temp.push([xInterval * i - ((xCount - 1) * xInterval) / 2, yInterval * j - ((yCount - 1) * yInterval) / 2, 0]);
      }
    }
    return temp;
  }, [xCount, yCount, xInterval, yInterval]);

  return (
    <group rotation={[Math.PI / 2, 0, 0]}>
      <Instances limit={xCount * yCount}>
        <boxGeometry args={nodeSize} />
        <meshStandardMaterial color={color} roughness={0.3} metalness={0.5} opacity={1} transparent={true} />
        {positions.map((position, i) => (
          <Instance key={i} position={position} />
        ))}
      </Instances>
    </group>
  );
};
