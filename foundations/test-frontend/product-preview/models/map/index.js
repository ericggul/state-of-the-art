...
const COLORS = {
  ...
  positional: "hsl(240, 100%, 30%)",
  attention: "hsl(240, 100%, 70%)",
  norm: "hsl(240, 100%, 50%)",
  ffn: "hsl(240, 100%, 30%)",
  output: "hsl(240, 100%, 70%)",
};
...
export default function Transformer3D() {
  return (
    ...
    >
      ...
      <pointLight position={[10, 10, 10]} color="hsl(240, 100%, 50%)" />
      <directionalLight position={[0, 10, 10]} intensity={30} color="hsl(240, 100%, 50%)" />
      <directionalLight position={[10, 0, 10]} intensity={30} color="hsl(240, 100%, 50%)" />

      {/* <ambientLight intensity={0.5} color="hsl(240, 100%, 50%)" /> */}
      ...
      <OrbitControls />
      {/* <EffectComposer>
        <Bloom intensity={1} luminanceThreshold={0.6} luminanceSmoothing={0.9} />
      </EffectComposer> */}
    </Canvas>
  );
}
  ...
  return (
    <animated.group position={position} scale={scale}>
      <mesh>
        ...
        <meshStandardMaterial color={color} roughness={0.1} metalness={1} opacity={opacity} transparent={true} depthTest={false} depthWrite={false} />
      </mesh>
    </animated.group>
  );
};
